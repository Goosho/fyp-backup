...
dataset = pd.read_csv("faisal_town.csv")
dataset['Aqi_PM']=0
dataset['Aqi_CO']=0
dataset['Aqi_NOx']=0
...
dataset.insert(6, column = "temp_date", value = 0)
dataset['temp_date'] = dataset['Date']
...
#        dataset['Humidity'][index] = dataset['Humidity'][index-1];
    ...
    if index <= 5 :
        ...
        if CO_obs >=CO_min[0]  and CO_obs <= CO_max[0]:
            value  = 0
        if CO_obs >=CO_min[1]  and CO_obs <= CO_max[1]:
            value  = 1
        if CO_obs >=CO_min[2]  and CO_obs <= CO_max[2]:
            value  = 2
        if CO_obs >=CO_min[3]  and CO_obs <= CO_max[3]:
            value  = 3
        if CO_obs >=CO_min[4]  and CO_obs <= CO_max[4]:
            value  = 4
	if CO_obs >CO_max[4]:
	    value = 4
    ...
    if index <= 5 :
        ...
        if NOx_obs >=NOx_min[0]  and NOx_obs <= NOx_max[0]:
            value  = 0
        if NOx_obs >=NOx_min[1]  and NOx_obs <= NOx_max[1]:
            value  = 1
        if NOx_obs >=NOx_min[2]  and NOx_obs <= NOx_max[2]:
            value  = 2
        if NOx_obs >=NOx_min[3]  and NOx_obs <= NOx_max[3]:
            value  = 3
	if NOx_obs> NOx_max[3]:
	    value = 3
#	print(value)
...
dataset['Date'] = dataset['temp_date']

dataset.drop(['PM1', 'PM10','month','minutes','temp_date'], axis='columns', inplace=True)
