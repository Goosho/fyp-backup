...
dataset = pd.read_csv("model_town.csv")
dataset['Aqi_PM']=0
dataset['Aqi_CO']=0
dataset['Aqi_NOx']=0
...
CO_min = [0,2.1,4.1,8.1,30.1]
CO_max = [2,4,8,30,100]

NOx_min = [0,3.1,7.55,37.55]
NOx_max = [3,7.5,37.5,15000]
...
for index,row in dataset.iterrows():
    ...
    if dataset['CO'][index]>100:
        dataset['CO'][index] = 100
...
#	 dataset['Temperature'][index] = 0
# 	 dataset['Humidity'][index] = 0
	...
	PM_obs = (dataset['PM2.5'][index-5] +dataset['PM2.5'][index-4] +dataset['PM2.5'][index-3] +dataset['PM2.5'][index-2] +dataset['PM2.5'][index-1] +dataset['PM2.5'][index-0])/6
        ...
        if PM_obs >=PM_min[0]  and PM_obs <= PM_max[0]:
            value  = 0
        elif PM_obs >=PM_min[1]  and PM_obs <= PM_max[1]:
            value  = 1
        elif PM_obs >=PM_min[2]  and PM_obs <= PM_max[2]:
            value  = 2
        elif PM_obs >=PM_min[3]  and PM_obs <= PM_max[3]:
            value  = 3
        elif PM_obs >=PM_min[4]  and PM_obs <= PM_max[4]:
            value  = 4
        elif PM_obs >=PM_min[5]  and PM_obs <= PM_max[5]:
            value  = 5
        elif PM_obs >= 500.5:
            value = 5
	else :
    ...
    if index <= 5 :
        dataset['Aqi_CO'][index] = dataset['CO'][index]
    else :
        CO_obs = (dataset['CO'][index-5] +dataset['CO'][index-4] +dataset['CO'][index-3] +dataset['CO'][index-2] +dataset['CO'][index-1] +dataset['CO'][index-0])/6
        if CO_obs >=CO_min[0]  and CO_obs <= CO_max[0]:
            value  = 0
        elif CO_obs >=CO_min[1]  and CO_obs <= CO_max[1]:
            value  = 1
        elif CO_obs >=CO_min[2]  and CO_obs <= CO_max[2]:
            value  = 2
        elif CO_obs >=CO_min[3]  and CO_obs <= CO_max[3]:
            value  = 3
        elif CO_obs >=CO_min[4]  and CO_obs <= CO_max[4]:
            value  = 4
	elif CO_obs >CO_max[4]:
	    value = 4
        else :
            value = 4
        value_1 = CO_obs - CO_min[value]
        value_2 = AQI_max[value] - AQI_min[value]
        value_3 = CO_max[value] - CO_min[value]
        dataset['Aqi_CO'][index] = ((value_1 * value_2)/value_3)+ AQI_min[value]



    if index <= 5 :
        dataset['Aqi_NOx'][index] = dataset['NOx'][index]
    else :
        NOx_obs = (dataset['NOx'][index-5] +dataset['NOx'][index-4] +dataset['NOx'][index-3] +dataset['NOx'][index-2] +dataset['NOx'][index-1] +dataset['NOx'][index-0])/6
        if NOx_obs >=NOx_min[0]  and NOx_obs <= NOx_max[0]:
            value  = 0
        elif NOx_obs >=NOx_min[1]  and NOx_obs <= NOx_max[1]:
            value  = 1
        elif NOx_obs >=NOx_min[2]  and NOx_obs <= NOx_max[2]:
            value  = 2
        elif NOx_obs >=NOx_min[3]  and NOx_obs <= NOx_max[3]:
            value  = 3
	elif NOx_obs> NOx_max[3]:
	    value = 3
        else :
            value = 3
#	print(value)
        value_1 = NOx_obs - NOx_min[value]
        value_2 = AQI_max[value] - AQI_min[value]
        value_3 = NOx_max[value] - NOx_min[value]
        dataset['Aqi_NOx'][index] = ((value_1 * value_2)/value_3)+ AQI_min[value]
